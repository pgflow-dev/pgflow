...
async function processBatchForFlow<TFlow extends AnyFlow>(
  ...
  const poller = new internal.flow.StepTaskPoller(
    ...
    abortController.signal,
    {
      ...
      maxPollSeconds: 2,     // Short poll - no need to match cron interval
...
serve(async (req) => {
  try {
    ...
    if (!flow) {
      return new Response(JSON.stringify({ error: `Unknown flow: ${flow_slug}` }), { 
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }

    logger.info(`Processing batch for flow: ${flow_slug}`, { batch_size, max_concurrent });
