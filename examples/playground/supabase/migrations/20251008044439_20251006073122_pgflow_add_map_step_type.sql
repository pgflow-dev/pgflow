...
-- MANUAL DATA MIGRATION: Prepare existing data for new constraints
-- This must run AFTER dropping NOT NULL but BEFORE adding new constraints

-- Update 1: Set remaining_tasks to NULL for 'created' status
...
-- Update 2: Backfill initial_tasks for 'started' steps
-- The new constraint "initial_tasks_known_when_started" requires that
-- initial_tasks is NOT NULL when status = 'started'
-- For existing started steps, initial_tasks should equal current remaining_tasks
UPDATE "pgflow"."step_states"
SET "initial_tasks" = COALESCE("remaining_tasks", 1)
