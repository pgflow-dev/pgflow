...
import { FlowRunProvider } from '@/components/flow-run-provider-new';
import FlowRunDetailsNew from '@/components/flow-run-details-new';
import WebsiteAnalysisUI from '@/components/website-analysis-ui';
import { useState } from 'react';
import { useFlowRun } from '@/components/flow-run-provider-new';

// Component that uses the new PgflowClient context
function RunPageContentNew() {
    ...
    <div className="flex flex-col lg:flex-row">
      ...
      <div className="w-full lg:w-[65%] xl:w-[70%] lg:pr-6 order-last lg:order-first">
        <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-md">
          <h2 className="text-green-800 font-medium">🚀 New PgflowClient Implementation</h2>
          <p className="text-green-700 text-sm mt-1">
            This page uses the new PgflowClient-based components with dynamic step discovery and real-time events.
          </p>
          {flowRun && (
            <p className="text-green-600 text-xs mt-2">
              FlowRun ID: {flowRun.run_id} | Status: {flowRun.status} | Remaining: {flowRun.remaining_steps}
            </p>
          )}
        </div>
        
        <WebsiteAnalysisUI
          runData={runData} // Legacy compatibility
          loading={loading}
          error={error}
...
export default function RunPageNew() {
