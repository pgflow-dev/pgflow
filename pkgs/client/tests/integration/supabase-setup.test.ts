...
describe('Supabase Setup Integration', () => {
  ...
  it('should have pgflow schema installed', withTransaction(async (sql) => {
  ...
  it('should have pgflow tables', withTransaction(async (sql) => {
  ...
  it('should connect to supabase client', async () => {
    ...
    // Note: pgflow schema may not be exposed via PostgREST by default
    expect(client.supabaseUrl).toBe('http://localhost:50521');
    expect(client.supabaseKey).toBeDefined();
    
    // Alternative test: verify the client instance is created successfully
