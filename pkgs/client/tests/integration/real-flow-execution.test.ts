...
describe('Real Flow Execution E2E', () => {
  it('polls task, completes it, and receives broadcast events', withPgNoTransaction(async (sql) => {
    ...
    // Grant table access for SupabaseBroadcastAdapter.fetchFlowDefinition
    await sql`GRANT SELECT ON TABLE pgflow.flows TO anon`;
    await sql`GRANT SELECT ON TABLE pgflow.steps TO anon`;
