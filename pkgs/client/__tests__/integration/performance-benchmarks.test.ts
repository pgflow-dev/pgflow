...
describe('Performance Benchmarks', () => {
  it(
    ...
    withPgNoTransaction(async (sql) => {
      ...
      for (const stepRecord of dbState) {
        ...
        expect(stepRecord.output.result).toBe(
          `${stepRecord.step_slug}-completed`
        );
      }
  ...
  it(
    ...
    withPgNoTransaction(async (sql) => {
      ...
      for (let i = 0; i < 5; i++) {
        await run
          .step(`root_${i}`)
          .waitForStatus(FlowStepStatus.Completed, { timeoutMs: 5000 });
      }
      ...
      for (let i = 5; i < 25; i++) {
        await run
          .step(`middle_${i}`)
          .waitForStatus(FlowStepStatus.Completed, { timeoutMs: 10000 });
      }
      ...
      const totalDuration = Date.now() - startTime;
      const avgStepTime =
        Object.values(stepTimes).reduce((a, b) => a + b, 0) / stepCount;
  ...
  it(
    ...
    withPgNoTransaction(async (sql) => {
      ...
      console.log(
        `=== Creating ${flowCount} flows with ${stepsPerFlow} steps each ===`
      );
      ...
      console.log(
        'Task breakdown by flow:',
        allAvailableTasks.reduce((acc, task) => {
      ...
      for (const flowSlug of flows) {
        ...
        const relevantTasks = tasks.filter((task) =>
          relevantRunIds.includes(task.run_id)
        );
        console.log(
          `Flow ${flowSlug}: ${relevantTasks.length} relevant tasks out of ${tasks.length}`
        );
      ...
      const totalDuration = Date.now() - startTime;
      const taskCompletionDuration =
        taskCompletionEndTime - taskCompletionStartTime;
  ...
  it(
    ...
    withPgNoTransaction(async (sql) => {
      ...
      console.log(
        `=== Initial memory usage: ${initialMemory.toFixed(2)} MB ===`
      );
      ...
      for (let i = 0; i < runCount; i++) {
        ...
        if (i % 10 === 9) {
          ...
          console.log(
            `Memory after ${i + 1} runs: ${currentMemory.toFixed(2)} MB`
          );
        }
      }
      ...
      for (let i = 0; i < runCount; i++) {
        const taskPromise = sqlClient
          .pollForTasks(testFlow.slug, 1, 5, 200, 30)
          .then((tasks) => {
            if (tasks.length > 0) {
      ...
      console.log(
        `Memory increase: ${(finalMemory - initialMemory).toFixed(2)} MB`
      );
      console.log(
        `Memory per run: ${((finalMemory - initialMemory) / runCount).toFixed(
          3
        )} MB`
      );
