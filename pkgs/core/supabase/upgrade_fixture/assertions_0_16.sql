...
-- The obsolete three-argument overload is gone after the upgrade: an
-- omitted queue_name argument is rejected outright
do $fix$
begin
  begin
    perform pgflow.start_tasks(
      'MixedCaseFlow',
      (select array_agg(msg_id)::bigint[] from fixture_claim),
      '11111111-1111-1111-1111-111111111111'::uuid
    );
    raise exception 'obsolete 3-argument start_tasks call still works after upgrade';
  exception
