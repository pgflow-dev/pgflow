-- Update "create_flow" to use NULL defaults with COALESCE
-- This allows callers to pass NULL to explicitly use the default value
CREATE OR REPLACE FUNCTION "pgflow"."create_flow" ("flow_slug" text, "max_attempts" integer DEFAULT NULL, "base_delay" integer DEFAULT NULL, "timeout" integer DEFAULT NULL) RETURNS "pgflow"."flows" LANGUAGE sql SET "search_path" = '' AS $$
WITH
  defaults AS (
...
-- Update "_create_flow_from_shape" to pass options from shape
