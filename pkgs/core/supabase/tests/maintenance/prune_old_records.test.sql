...
values ('11111111-1111-1111-1111-111111111111', 'test_queue', 'test_function', now());

-- Start and complete two flows
...
-- Verify the setup: we should have 5 runs, 5 step_states, 5 step_tasks, and 1 worker
select is(
...
select is(
  ...
  5::bigint,
  'Should have 5 step_states'
);

select is(
  (select count(*) from pgflow.step_tasks),
  5::bigint,
  'Should have 5 step_tasks'
);
...
-- PRUNE OLD RECORDS
-- this should not prune anything as all records are recent
select pgflow.prune_old_records(7);

-- TEST: Run pruning with a 7-day retention - nothing should be pruned because all records are recent
select is(
  (select count(*) from pgflow.workers),
  1::bigint,
  'No workers should be pruned with 7-day retention'
);

select is(
  (select count(*) from pgflow.step_tasks),
  5::bigint,
  'No step_tasks should be pruned with 7-day retention'
);

select is(
  (select count(*) from pgflow.step_states),
  5::bigint,
  'No step_states should be pruned with 7-day retention'
);

select is(
  (select count(*) from pgflow.runs),
  5::bigint,
  'No runs should be pruned with 7-day retention'
);
...
-- Execute pruning function (ignore return values)
...
select is(
  ...
  array['flow_that_completed_recently', 'flow_that_failed_recently', 'flow_that_runs'],
