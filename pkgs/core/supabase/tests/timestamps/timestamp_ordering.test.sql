...
select pgflow.add_step('timestamp_test', 'failure_step', max_attempts => 0);
select pgflow.add_step('timestamp_test', 'dependent_step', deps => ARRAY['success_step']);
...
select ok(
  (select completed_at > created_at from pgflow.step_states 
   where run_id = :'run_id' and step_slug = 'success_step'),
...
select ok(
  (select failed_at > created_at from pgflow.step_states 
   where run_id = :'run_id' and step_slug = 'failure_step'),
...
select ok(
  (select started_at > created_at from pgflow.step_states 
   where run_id = :'run_id' and step_slug = 'dependent_step'),
