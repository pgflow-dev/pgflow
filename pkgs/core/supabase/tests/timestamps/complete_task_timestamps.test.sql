...
select pgflow.complete_task((select run_id from pgflow.runs limit 1), 'second', 0, '{"result": "second completed"}'::jsonb);
select pgflow.complete_task((select run_id from pgflow.runs limit 1), 'third', 0, '{"result": "third completed"}'::jsonb);
