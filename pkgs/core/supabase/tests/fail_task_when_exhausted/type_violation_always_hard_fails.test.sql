-- Test: TYPE_VIOLATION in complete_task always hard fails regardless of when_exhausted
-- TYPE_VIOLATION is a programming error (wrong return type), not a runtime condition
-- It should always cause the run to fail, even with when_exhausted='skip' or 'skip-cascade'
begin;
select plan(4);
select pgflow_tests.reset_db();

-- SETUP: Create a flow where step_a feeds into a map step
-- step_a has when_exhausted='skip-cascade' but TYPE_VIOLATION should override this
select pgflow.create_flow('test_flow');
select pgflow.add_step('test_flow', 'step_a', when_exhausted => 'skip-cascade');
...
select is(
  ...
  'failed',
  'step_a should be failed on TYPE_VIOLATION (not skipped despite when_exhausted=skip-cascade)'
);
