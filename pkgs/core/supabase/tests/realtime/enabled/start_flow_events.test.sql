...
-- Start flow with realtime enabled and capture run_id
with flow as (
  select * from pgflow.start_flow('sequential', '{"test_data": "value"}'::jsonb, realtime => 'true')
)
...
select is(
  ...
  1::int,
  'pgflow.start_flow with realtime enabled should send exactly one run:started event'
);
...
-- Test 7: Verify topic formatting (default channel)
select is(
