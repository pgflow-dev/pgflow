...
-- spelling in pgmq (#650). PGMQ's public message API normalizes names, so
-- every pgflow message operation addresses the physical queue by the stored
-- canonical name directly: dispatch, claim, visibility, completion, retry,
-- recovery. No queue listing is consulted and no second metadata entry is
-- created.
begin;
select plan(13);
...
-- The claim requires the queue's canonical identity, not the polled
-- spelling: messages are read from the physical 'LegacyCase' queue, but
-- tasks store 'legacycase', and the match is exact. Passing the polled
-- mixed-case spelling claims nothing (#650).
select pgflow_tests.ensure_worker('legacycase');
select pgflow_tests.ensure_worker('LegacyCase');

select array_agg(msg_id) as ids into temporary legacy_msgs
from pgmq.read_with_poll('LegacyCase', 30, 5, 1, 50);

select is(
  (select count(*) from pgflow.start_tasks(
    'LegacyCase',
    (select ids from legacy_msgs),
    '11111111-1111-1111-1111-111111111111'::uuid,
    'LegacyCase'
  ))::int,
  0,
  'claim with the polled mixed-case spelling matches nothing'
);
...
select is(
  ...
  1,
  'claim with the canonical stored name claims the task'
);
