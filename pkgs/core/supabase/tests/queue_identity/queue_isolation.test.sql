...
-- Defaulted queue name (released signature) was removed: the omitted
-- argument is rejected outright, and an explicit NULL claims nothing
-- (no silent fallback to a reconstructed queue)
select throws_ok(
  ...
  'function pgflow.start_tasks(unknown, bigint[], uuid) does not exist',
  'omitted queue_name argument is rejected'
);
...
select is(
  ...
  0,
  'explicit NULL queue name claims nothing instead of falling back'
);
...
select is(
  ...
  2,
  'explicit queue name claims both iso_a tasks'
);
...
select is(
  ...
  , 'iso_b'))::int,
  1,
  'the valid task in the batch is claimed through its queue'
);
