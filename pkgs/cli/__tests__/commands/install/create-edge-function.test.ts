...
describe('createEdgeFunction', () => {
  ...
  it('should inject package version instead of @latest in deno.json', async () => {
    ...
    expect(denoJson.imports['@pgflow/core']).toBe(`jsr:@pgflow/core@${version}`);
    expect(denoJson.imports['@pgflow/dsl']).toBe(`jsr:@pgflow/dsl@${version}`);
    expect(denoJson.imports['@pgflow/edge-worker']).toBe(`jsr:@pgflow/edge-worker@${version}`);

    // Verify subpath exports don't have versions (they shouldn't)
    expect(denoJson.imports['@pgflow/core/']).toBe('jsr:@pgflow/core/');
    expect(denoJson.imports['@pgflow/dsl/']).toBe('jsr:@pgflow/dsl/');
    expect(denoJson.imports['@pgflow/edge-worker/']).toBe('jsr:@pgflow/edge-worker/');
  });
});
