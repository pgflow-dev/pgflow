...
VENDOR_DIR="$CLI_DIR/supabase/functions/_vendor"

echo "🔄 Syncing edge function dependencies for CLI e2e tests..."
...
echo "📋 Copying migrations from @pgflow/core..."
MIGRATIONS_DIR="$CLI_DIR/supabase/migrations"
mkdir -p "$MIGRATIONS_DIR"
rm -f "$MIGRATIONS_DIR"/*.sql
cp "$MONOREPO_ROOT/pkgs/core/supabase/migrations"/*.sql "$MIGRATIONS_DIR/"

echo "✅ Dependencies synced to $VENDOR_DIR"
echo "✅ Migrations synced to $MIGRATIONS_DIR"
