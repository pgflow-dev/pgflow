...
serve(async (req) => {
  ...
  try {
    ...
    const contextText = rankedResults
      .map((r: any) => {
        const source = r.source === 'knowledge_base' ? '📚 Knowledge'
          : r.source === 'web_search' ? '🌐 Web'
          : '🧠 Memory';
...
2. **Parallel Processing** (3 simultaneous pipelines):
   - **Semantic Search**: Formulates a semantic query → searches knowledge base with pgvector
   - **Web Search**: Extracts search queries → performs web searches
   - **Memory Search**: Formulates memory queries → searches user-specific memories
