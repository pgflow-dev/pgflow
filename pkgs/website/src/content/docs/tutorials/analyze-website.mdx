...
4-step ETL: scrape → 2×AI → Postgres
...
:::tip[Why pgflow?]
pgflow stores workflow DAGs, retry logic, and execution history directly in Postgres.
This means simpler infrastructure and built-in observability.
...
| Symptom | Fix |
|---------|-----|
| `Error: SASL_SIGNATURE_MISMATCH` | Check if DB password is URL-encoded ([Prepare DB Connection URL](/getting-started/install-pgflow/#prepare-db-connection-url)) |
| `Missing OPENAI_API_KEY` | Export env var in `supabase/.env` and restart `functions serve` |

### Compile-time Issues
| Symptom | Fix |
|---------|-----|
| Workers not running | Ensure you've run the compilation step and applied migrations correctly ([Compile Guide](/getting-started/compile-to-sql/)) |

### Runtime Issues
| Symptom | Fix |
|---------|-----|
| Row not created | Check `SUPABASE_SERVICE_ROLE_KEY` – needs *service-role* level |
| `TypeError: fetch failed` | Check internet connection – Docker containers need network access to reach OpenAI API |
