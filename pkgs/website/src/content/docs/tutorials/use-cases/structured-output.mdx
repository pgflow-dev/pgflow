...
export default new Flow<Input>({ slug: 'processProduct' })
  .step({ slug: 'extract' }, (flowInput) =>
    extractProduct(flowInput.text)
  )
  .step({ slug: 'enrich', dependsOn: ['extract'] }, (deps) =>
    enrichProduct(deps.extract)
  )
  .step({ slug: 'save', dependsOn: ['enrich'] }, (deps) =>
    saveProduct(deps.enrich)
  );
```
