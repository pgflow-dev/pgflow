...
export default new Flow<Input>({ slug: 'ragSearch' })
  .step({ slug: 'transform' }, (flowInput) =>
    transformQuery(flowInput.query)
  )
  .step({ slug: 'retrieve', dependsOn: ['transform'] }, (deps) =>
    retrieveDocuments({
      query: deps.transform.enhanced,
      limit: 20,
    })
  )
  .step({ slug: 'rerank', dependsOn: ['retrieve', 'transform'] }, async (deps, ctx) => {
    const flowInput = await ctx.flowInput;
    return rerankResults({
      query: flowInput.query,
      documents: deps.retrieve,
    });
  })
  .step({ slug: 'answer', dependsOn: ['rerank'] }, async (deps, ctx) => {
    const flowInput = await ctx.flowInput;
...
Deploy the flow by importing it into a worker Edge Function and starting the worker. On first start, the worker compiles the flow definition into the database and verifies it on every restart - see [Startup Compilation](/concepts/startup-compilation/).
