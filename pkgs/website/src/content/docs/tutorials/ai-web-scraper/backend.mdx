...
Put these in `supabase/functions/_tasks` (see [organizing flows code](/build/authoring/organize-flow-code/) for project structure):
...
See more in [Monitor flow execution](/deploy/observe/monitor-execution/) documentation page
:::
...
For debugging, see [Monitor flow execution](/deploy/observe/monitor-execution/).
