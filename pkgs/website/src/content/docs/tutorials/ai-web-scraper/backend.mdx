...
Put these in `supabase/functions/_tasks` (see [organizing flows code](/develop/authoring/organize-flow-code/) for project structure):
...
Connect tasks into a workflow using pgflow's [TypeScript DSL](/concepts/flows/understanding-flows/) (`supabase/functions/_flows/analyze_website.ts`):
...
See more in [Monitor flow execution](/operate/observe/monitor-execution/) documentation page
:::
...
For debugging, see [Monitor flow execution](/operate/observe/monitor-execution/).
