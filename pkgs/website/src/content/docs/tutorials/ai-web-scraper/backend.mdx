...
Connect tasks into a workflow (`supabase/functions/_flows/analyze_website.ts`):
...
:::tip[Runs in parallel]
Summary and tags execute simultaneously since both only need website content. Cuts time in half!
:::
...
Turn your TypeScript flow into SQL:
...
:::tip[Pure SQL execution]
The compiler converts your flow into native PostgreSQL functions. No runtime overhead - just SQL executing inside your database!
:::

:::danger[Adding or removing steps]
Changing steps requires a new flow with unique `slug`. See [versioning](/how-to/version-your-flows/).
:::
...
Create an Edge Worker to run your flow:
