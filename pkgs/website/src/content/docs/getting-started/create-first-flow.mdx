...
2. ### Create task implementations
    ...
    <Tabs>
      ...
      // Test URL that will trigger the failure simulation
      const FAILURE_URL = 'https://test-failure.example.com';
      
      /**
       * Simulates a failure for testing error handling
       * Only fails if the URL matches the test failure URL
       */
