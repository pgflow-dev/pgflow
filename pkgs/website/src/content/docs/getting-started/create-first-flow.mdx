...
- Completed the [pgflow installation](/pgflow/getting-started/install-pgflow/)
...
5. ### Verify flow creation
    ...
    Let's verify the flow was created correctly:

    ```bash frame="none"
    npx supabase db query "SELECT * FROM pgflow.flows WHERE flow_slug = 'analyze_website'"
    ```
...
<Aside>
Now your workflow is defined in the database and ready to be executed. The next step is to [set up a worker and trigger the flow](/pgflow/getting-started/trigger-and-monitor/).
</Aside>
