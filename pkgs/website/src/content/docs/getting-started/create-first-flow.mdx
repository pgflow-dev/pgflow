...
2. ### Create the flow definition
    ...
    export default new Flow<Input>({
      ...
      .step(
        { slug: 'greet', dependsOn: ['format_name'] },
        (input) => `Hello, ${input.format_name}!`
      );
    ```
    ...
    :::important[JSON Serialization Requirement]
...
2. **Step 1 (format_name)**: Combines the first and last name to create a full name.
3. **Step 2 (greet)**: Takes the formatted name and creates a greeting.
