...
<CardGrid>
  <LinkCard 
    title="maxAttempts" 
    description="Maximum retry attempts before marking a task as failed. Can be set at flow or step level."
    href="#maxattempts"
  />
  <LinkCard 
    title="baseDelay" 
    description="Initial retry delay in seconds for exponential backoff. Can be set at flow or step level."
    href="#basedelay"
  />
  <LinkCard 
    title="timeout" 
    description="Visibility timeout in seconds. How long a task stays invisible while being processed."
    href="#timeout"
  />
  <LinkCard 
    title="Worker Options" 
    description="Runtime settings like concurrency, connections, and polling behavior for the EdgeWorker."
...
These settings are defined in your TypeScript flow code and compiled into SQL migrations. They control how individual steps are executed and retried. Set defaults at the flow level, override for specific steps. Step-level options are `null` by default, inheriting from flow-level settings.
