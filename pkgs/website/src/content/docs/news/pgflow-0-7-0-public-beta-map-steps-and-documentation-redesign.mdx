...
pgflow 0.7.0 is here - a major milestone and long-awaited release that brings parallel array processing,
...
**[Build](/build/)** - Everything related to authoring workflows:
- Four comprehensive guides for starting flows (TypeScript client, Supabase RPC, pg_cron, database triggers)
- Practical patterns for organizing code and processing arrays
- Flow management (versioning, deletion)

**[Deploy](/deploy/)** - Production deployment and operations:
- Supabase-specific deployment guides
- Monitoring workflow execution and worker health
- Maintenance tasks (updates, tuning, data pruning)

**[Concepts](/concepts/)** - Deeper understanding:
- Expanded with detailed guides on map steps, context object, data model
- New overview page organizing concepts by topic
- Visual explanations with diagrams and examples

**[Reference](/reference/)** - Technical specifications:
- Complete API documentation for the TypeScript client
- Workflow context object reference
...
- **Animated DAG visualization** in the hero, showing pgflow's workflow execution in action
- **Before/after comparison** with scrollable code examples demonstrating "Without pgflow" vs "With pgflow"
- Interactive showcase of what pgflow handles for you
...
This independent retry isolation makes workflows more efficient and resilient. Each task has its own
...
**Fixed config.toml Corruption** - The CLI no longer corrupts `config.toml` files with minimal configurations. This was fixed by switching to a maintained fork of toml-patch (`@decimalturn/toml-patch`) that properly handles edge cases.
...
- Fixed data pruning foreign key constraint bug preventing cleanup operations
...
- Improved pre-migration check clarity with better error messages
...
<details>
<summary>⚠️ Pre-Migration Check Query (click to expand) ⚠️</summary>
...
- ⚠️ These are expected data migrations handled automatically
- 🆘 Unexpected rows or errors? Share output on Discord for help
