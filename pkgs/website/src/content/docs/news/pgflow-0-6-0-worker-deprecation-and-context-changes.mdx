...
Workers detect deprecation within 5 seconds via heartbeat and gracefully stop polling. The [deployment guide](/operate/deploy/update-deployed-flows/) has been simplified with a single safe deployment sequence.
...
3. Use [worker deprecation](/operate/deploy/update-deployed-flows/) for smooth deployments
