...
```typescript
new Flow<{ first_name: string; last_name; string }>({
  slug: 'greet_user',
})
  .step(
    { slug: 'full_name' },
...
// when you run this flow with { first_name: 'Alice', last_name: 'Smith' },
// it will return { greeting: 'Hello, Alice Smith!' } (the last step)
```
