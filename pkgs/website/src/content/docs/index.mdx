...
new Flow<{ url: string }>({ slug: 'analyzeArticle' })
  ...
  .step({ slug: 'summarize', dependsOn: ['scrape'] },
    (input) => summarizeContent(input.scrape)
  )
  .step({ slug: 'extractKeywords', dependsOn: ['scrape'] },
    (input) => extractKeywords(input.scrape)
  )
...
<CardGrid>
  ...
  <Card title="👁️ Build and test steps in isolation">
