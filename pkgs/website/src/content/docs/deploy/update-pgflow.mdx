...
The 0.17.0 queue-identity migration is a breaking maintenance upgrade: it drops the three-argument `start_tasks()` claim that old workers still use. Do not run this step for that update. Apply it only inside the [Queue identity and private step queues (0.17.0)](#queue-identity-and-private-step-queues-0170) sequence below, with every worker stopped and the packages updated first.
</Aside>
...
## Queue identity and private step queues (0.17.0)
...
The migration changes the `pgflow.start_tasks()` claim signature: the `queue_name` argument is now **required** (the released three-argument form and the NULL default are gone), and `startTasks()` on `IPgflowClient`/`PgflowSqlClient` requires the queue argument in TypeScript as well. The argument is the task's **canonical persisted route**: `lower(flow_slug)` in flow mode, or the generated step route in step mode. A direct step-mode claim also needs the exact `step_slug`. pgflow's own workers poll and claim through those values, so existing flows keep working unchanged, including queues that PGMQ still lists with an older mixed-case spelling. Custom code that calls `pgflow.start_tasks()` or `startTasks()` directly with the old signature breaks and must pass the canonical route explicitly; passing the polled mixed-case spelling of an upgraded queue matches nothing. There is **no rolling mixed-version upgrade**: old workers cannot claim tasks against the new schema, and new workers cannot run against the old schema. Deploy matching pgflow packages, workers, and explicit-route callers together, while everything is stopped.
</Aside>
...
8. ### Deploy matching packages and workers
   ...
   Update the whole pgflow package set to the same version - npm packages, the JSR edge-worker import, and any custom code that calls `start_tasks()`/`startTasks()` explicitly (it must now pass the task's canonical stored route, plus `step_slug` for a step-mode flow) - and deploy the new worker code.
...
If this update crosses 0.16.0 to 0.17.0, do not apply the migration here: follow the [Queue identity and private step queues (0.17.0)](#queue-identity-and-private-step-queues-0170) maintenance sequence, even for a local database, with workers stopped.
