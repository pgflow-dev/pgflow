...
pgflow 0.17.0 stores each step's and task's physical queue identity in the database: `pgflow.steps.queue_name` and `pgflow.step_tasks.queue_name`, always the canonical lowercase `lower(flow_slug)` for now. A task's message identity is `(queue_name, message_id)` instead of `message_id` alone, so the same message id in two queues can never collide.

This is a maintenance upgrade from 0.16.0: no online-upgrade guarantee, no mixed-version rolling upgrade. Follow the fenced procedure:

1. **Stop and drain workers**: disable each pgflow worker (edge function or process worker) and wait until no tasks are `started`.
2. **Pause writers**: pause flow producers (`pgflow.start_flow` callers), definition writes (worker startups, `create_flow`/`add_step`), and maintenance/recovery jobs (`requeue_stalled_tasks`, the pruning cron) so no queue operations run during the migration.
3. **Apply the migration** through Supabase's migration runner (`npx supabase migrations up`, or your project's migration pipeline). The migration is transactional and uses bounded lock waits. If your database contains conflicting data (two flows whose slugs differ only by case, duplicate `(queue, message_id)` pairs, or queue names longer than PGMQ's limit), it fails and leaves the database unchanged. Resolve the conflict manually (pick one spelling, deduplicate, or rename before migrating); pgflow never renames or deletes definitions for you.
4. **Update the pruning helper** if you installed the optional `pgflow.prune_data_older_than()`: replace it with the current version from [Prune Old Data](/deploy/prune-records/). Adapt any customized copies yourself.
5. **Deploy matching packages**: update the whole pgflow package set to the same version (npm packages and the JSR edge-worker import).
6. **Resume**: re-enable workers, producers, and maintenance jobs.

Queues created by older releases keep their original mixed-case spelling in pgmq and continue to work through it; pgflow resolves the spelling through `pgmq.list_queues()` and never creates a second metadata entry.
