...
This release removes `compileFlow()`, ControlPlane, `pgflow compile`, and the `FlowWorkerConfig.compilation` option. It also replaces `pgflow.ensure_flow_compiled(text, jsonb, boolean)` with a two-argument startup-only signature. Old workers cannot start after the database migration.
...
For process workers, stop the processes and external supervisors before applying the migration. Update the packages and database, then restart the supervisors.
...
Replace `your-project-ref` with your Supabase project reference. The remote delete command is required only when the ControlPlane function was deployed.

Also remove any `[functions.pgflow]` block, every `pgflow compile` script or CI step, and every worker `compilation` property. Keep existing generated flow migration files as immutable migration history; do not delete or rewrite migrations that a database already applied.
