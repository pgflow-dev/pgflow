...
The one-time upgrade procedure lives in the [pgflow 0.16.0 release notes](/news/pgflow-0-16-0-startup-only-flow-compilation/): disable and drain each HTTP worker function, upgrade the pgflow packages together, apply the migration with workers stopped, then redeploy and restore `enabled` values.
