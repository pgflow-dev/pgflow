...
pgflow 0.17.0 changed how the helper addresses PGMQ: message cleanup now goes through each task's stored queue snapshot (`step_tasks.queue_name`) and archive cleanup through persisted definition routes. If you installed an earlier copy, replace it by re-running the SQL below (the `create or replace function` statement overwrites the old body). If you customized your copy, adapt the highlighted changes yourself: group message cleanup by `step_tasks.queue_name`, resolve names through `pgflow._effective_queue_name()`, and walk archive tables from `pgflow.steps.queue_name`.
</Aside>
