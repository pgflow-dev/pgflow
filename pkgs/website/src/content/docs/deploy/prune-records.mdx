...
- PGMQ messages - active queues addressed through each task's stored queue name (`step_tasks.queue_name`) and archive tables through persisted queue routes (`pgflow.steps.queue_name`)
...
<Aside type="note" title="Updating an installed copy">
pgflow 0.17.0 changed how the helper addresses PGMQ: message cleanup now goes through each task's stored queue snapshot (`step_tasks.queue_name`), and archive cleanup walks persisted definition routes (`pgflow.steps.queue_name`), deriving each archive table name through `pgmq.format_table_name()` - which lowercases the route itself, so no queue listing is consulted. If you installed an earlier copy, replace it by re-running the SQL below (the `create or replace function` statement overwrites the old body). If you customized your copy, adapt the changes yourself: group message cleanup by `step_tasks.queue_name` (PGMQ message operations normalize names themselves), and walk archive tables from `pgflow.steps.queue_name` via `pgmq.format_table_name()`.
</Aside>
