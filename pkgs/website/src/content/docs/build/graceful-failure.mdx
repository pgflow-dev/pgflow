...
The `retriesExhausted` option controls what happens when a step fails after exhausting all retry attempts. Instead of failing the entire run, you can skip the step and continue.
...
.step({
  ...
  maxAttempts: 3,
  retriesExhausted: 'skip',  // If email fails 3x, skip and continue
}, async (input) => {
...
```typescript
// This ALWAYS fails the run, even with retriesExhausted: 'skip'
.step({
  slug: 'fetchItems',
...
When `retriesExhausted: 'skip'` triggers, the step gets `skip_reason: 'handler_failed'` with the original error preserved in `error_message`.
...
You can use `retriesExhausted` together with `if` conditions for maximum flexibility:
...
.step({
  ...
  maxAttempts: 3,
  retriesExhausted: 'skip',          // If API fails, continue anyway
}, async (input) => {
...
**Do use `retriesExhausted: 'skip'` for:**
