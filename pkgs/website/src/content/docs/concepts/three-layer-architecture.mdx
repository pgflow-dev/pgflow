...
worker: "Worker\nTask Execution"
worker.class: warning

dsl -> core: Step definitions
dsl -> worker: Handler functions {
  style.stroke-dash: 3
}
core -> worker: Tasks
worker -> core: Results
...
**Definition-time** - Write flows in TypeScript; workers deploy the definition to the database at startup ([startup compilation](/concepts/startup-compilation/))
...
- Translation from high-level concepts to DAG primitives
...
**DSL → SQL Core**
- DSL translates to step definitions with clear semantics
