...
```typescript ins="{ sql, serviceSupabase, shutdownSignal }"
// Clean and focused on what's being used
async function syncData(input, { sql, serviceSupabase, shutdownSignal }) {
  const data = await sql`SELECT * FROM pending_sync`;
