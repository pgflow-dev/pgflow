...
async function handler(input: Input, context: Context) {
  ...
  console.log(`Enqueued at ${context.rawMessage.enqueued_at}`);
}
```
...
const MyFlow = new Flow({ slug: 'my_flow' })
  .step({ slug: 'process' }, async (input, context) => {
    ...
    console.log(`Flow: ${context.stepTask.flow_slug}`);
    console.log(`Message ID: ${context.stepTask.msg_id}`);
    // context.stepTask.input is the same as the input parameter
  });
```
