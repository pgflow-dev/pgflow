...
- `step_states` - Tracks each step's **status and dependencies**
...
A workflow run completes when every step reaches a terminal status. Each step in `pgflow.step_states` follows this lifecycle:
...
When you call `pgflow.start_flow()`, you only see step progression. That's the main mental model.

### How workers execute steps

Behind the scenes, when a step becomes ready, workers handle the execution by calling simple SQL functions. The implementation details—queues, tasks, coordination—are handled transparently by the SQL Core.

<Aside type="note" title="Implementation detail">
If you're just calling `start_flow()` and checking results, you don't need to know about tasks, queues, or worker coordination. Those are handled automatically by pgflow's SQL Core.
</Aside>
