...
2. The task state machine (`queued → started → completed/failed`)
3. Why type safety and coordination matter
...
1. **Worker polls for messages** `read_with_poll(queue, vt, qty)`
   • Very fast operation that locks & returns up-to-`qty` queue message records
   • Returns message records, no task payloads yet
   • Separate from task starting - pure message coordination
2. **Worker starts tasks** `start_tasks(msg_ids[], worker_id)`
   • Finds tasks with `status = 'queued'` matching the message IDs
   • Marks tasks as `started` with `started_at` timestamp and `worker_id`
   • Builds an *input object* that merges `run.input` with outputs of completed deps
   • Returns the task records with inputs
3. Worker runs your handler function.
4. On success → `complete_task(run_id, step_slug, task_index, output)`
   On error   → `fail_task(run_id, step_slug, task_index, error)`
5. SQL Core, in the *same commit*:
   - stores output / error
...
Task states flow through: `queued` → `started` → `completed`/`failed`. The `started` state tracks which tasks are actively being processed, preventing duplicate execution and enabling crash recovery.

That's it—**three SQL calls** advance the entire graph.
