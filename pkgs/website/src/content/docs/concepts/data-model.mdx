...
pgflow's data model separates workflow definitions from runtime execution state. Workflow orchestration uses PostgreSQL tables and functions in the `pgflow` schema for transparent, queryable behavior.
...
The schema organizes tables into two distinct categories that serve different purposes in the workflow lifecycle. All tables live in the `pgflow` schema.

### Static Definition Tables

These tables store the workflow structure - the "blueprint" of what workflows exist and how they connect:

**`flows`** - Workflow identity and global configuration
- Contains `max_attempts`, `base_delay`, `timeout`
...
Populated during deployment through migrations. Change only when workflow definitions change.

### Runtime State Tables

These tables track the execution state of workflow instances:
...
- Stores aggregated output when step completes
