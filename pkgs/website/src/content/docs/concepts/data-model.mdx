...
Flows and steps are identified by slugs - simple text identifiers like `'analyzeWebsite'` or `'fetchData'`. A slug starts with a letter, uses letters, numbers, and single internal underscores, ends with a letter or number, and has at most 128 characters. `run`, leading or trailing underscores, and `__` are invalid. Flow slugs are unique without regard to case; step slugs are unique without regard to case within one flow.
...
-- Definition tables
flows:       (flow_slug)
steps:       (flow_slug, step_slug) + denormalized queue_name
deps:        (flow_slug, dep_slug, step_slug)

-- Runtime tables
runs:        (run_id) + denormalized flow_slug
step_states: (run_id, step_slug) + denormalized flow_slug
step_tasks:  (run_id, step_slug, task_index) + denormalized flow_slug, queue_name
```
...
- Stores `queue_name`, the queue snapshot the task's message was dispatched to; the `(queue_name, message_id)` pair identifies the message and is unique per queue
