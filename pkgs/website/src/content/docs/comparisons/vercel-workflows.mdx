...
- **Startup deployments** - Worker startup compiles or verifies flow definitions automatically
...
pgflow puts **PostgreSQL at the center** of your workflow orchestration. Flows are defined in TypeScript and deployed by [worker startup compilation](/concepts/startup-compilation/), with all orchestration logic running **directly in the database**. The database decides when tasks are ready to execute based on explicit dependencies.
...
new Flow<{ userId: string }>({
  ...
  .step(
    { slug: 'createUser' },
    (flowInput) => ({ id: crypto.randomUUID(), email: flowInput.userId })
  )
  .step(
    { slug: 'sendWelcome', dependsOn: ['createUser'] },
    (deps) => sendEmail(deps.createUser.email, 'Welcome!')
  )
  .step(
    { slug: 'sendCheckin', dependsOn: ['sendWelcome', 'createUser'], startDelay: 604800 },
    (deps) => sendEmail(deps.createUser.email, 'How are you doing?')
  );
```
