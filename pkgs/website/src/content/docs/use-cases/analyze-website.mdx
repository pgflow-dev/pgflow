...
These go in `supabase/functions/_tasks` so they're available to the Edge Function workers.

The first task is a **web scraper** that fetches and cleans HTML content:
...
Next come two AI processing tasks that both use OpenAI. The first generates a concise summary:
...
The second AI task extracts relevant keywords:
...
Finally, a task to save the results to the database:
