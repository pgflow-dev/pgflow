...
You'll create a workflow with 4 tasks (exposed as 4 pgflow steps) that fetches a web page, processes it with **two parallel AI operations** (summary and keyword extraction), and writes everything into your Postgres database. By the end, you'll see real-time console logs and a neatly stored row in `public.websites`.
...
:::tip[Why pgflow?]
Unlike traditional solutions that need separate job queues and workers, pgflow stores the entire workflow DAG, retry logic, and execution history directly inside Postgres. This means simpler infrastructure, stronger consistency guarantees, and built-in observability.
:::
...
-- Add optional index for user-based queries
...
The regex-based HTML sanitization is simplified for clarity. For production, use a dedicated library like [html-to-text](https://github.com/html-to-text/node-html-to-text) or DOMPurify in browser environments to properly handle complex pages, nested elements, and encoding issues.
:::
</details>
...
:::tip[OpenAI Structured Outputs]
These functions use OpenAI's [Structured Outputs](https://platform.openai.com/docs/guides/structured-outputs) feature with [Zod schemas](https://zod.dev/) to:
- Ensure LLM responses are properly structured and typed
- Validate output against a defined schema 
- Save parsing/validation code by leveraging `zodTextFormat`

This approach creates robust, type-safe interactions with AI services that will surface validation errors rather than fail silently with incorrect data structures.
:::
...
<details open="false">
  <summary>💡 **Design pattern: How to name your steps effectively**</summary>
...
pgflow automatically runs the summary and tags steps in parallel since they only depend on the website content and not on each other. This cuts execution time nearly in half and it isolates failures - if one AI call fails, only that specific step retries.
:::
...
## Step 5 - Create a worker function

For your flow to run, you need to create an Edge Function worker:
...
:::note
This JWT verification is disabled for local development only. In production, you would typically keep JWT verification enabled and start the worker with a token. For more details on worker setup and configuration, see the [Run Flow guide](/getting-started/run-flow/).
:::
...
## Step 6 - Run the stack and test the flow
...
# Terminal 3 - start the worker (this terminal stays open showing logs)
...
```sql
-- Use uuid_generate_v4() to generate a random UUID for user_id in real applications
select * from pgflow.start_flow(
  flow_slug => 'analyze_website',
  input     => '{"url":"https://supabase.com","user_id":"00000000-0000-0000-0000-000000000000"}'
);
```

:::note[Execution Flow]
The workers execute `scrapeWebsite` first, then run both AI steps in parallel, and finally save the results once all analysis is complete. The database maintains a complete execution record.
:::
