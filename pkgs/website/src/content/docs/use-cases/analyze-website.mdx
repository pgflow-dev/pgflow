...
- `scrapeWebsite.ts` - Fetch webpage content
- `summarizeWithAI.ts` - Generate AI summary
- `extractTags.ts` - Extract keyword tags
- `saveWebsite.ts` - Save to database
...
The regex-based HTML sanitization is simplified for clarity. For production, use a dedicated library like [html-to-text](https://github.com/html-to-text/node-html-to-text) to properly handle complex pages, nested elements, and encoding issues.
:::
</details>
...
This approach creates robust, type-safe interactions with AI services that will surface validation errors rather than fail silently with incorrect data structures. Explore Zod's [documentation](https://zod.dev/?id=introduction) to create more complex validation schemas.
:::
...
:::note
This is a simplified setup for local development. For more details on worker setup and configuration, see the [Run Flow guide](/getting-started/run-flow/).
:::
