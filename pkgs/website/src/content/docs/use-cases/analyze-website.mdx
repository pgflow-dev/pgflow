...
This function asks GPT-4o to generate a concise summary of the webpage content, returning a single paragraph of text. It uses OpenAI's Structured Output feature with Zod to ensure the response has the expected structure.
...
This function analyzes the content to extract 5-10 relevant keyword tags that describe the webpage's topics or themes. The Zod schema enforces that the response is an array of strings with a maximum of 10 items.
