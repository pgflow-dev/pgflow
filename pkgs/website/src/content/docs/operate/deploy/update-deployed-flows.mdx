...
If you haven't deployed to production yet, see [Deploy Your First Flow](/operate/deploy/deploy-first-flow/) for initial setup instructions.
</Aside>
...
4. **Keep workers up**
   ...
   [Set up the pg_cron safety net](/operate/deploy/keep-workers-running/) for reliable production operation.
   ...
   **Your workers will eventually stop working without a safety net.** This is especially critical for production deployments where workflow reliability matters.
   ...
   The safety net takes 2 minutes to set up and prevents hours of debugging why workflows stopped running. [Configure it now →](/operate/deploy/keep-workers-running/)
   </Aside>
