...
Deno.test('maxConcurrent option is respected', withTransaction(async (sql) => {
  ...
  try {
    ...
    const messages = await waitFor(
      async () => {
        ...
        return archivedMessages.length >= MESSAGES_TO_SEND && archivedMessages;
      },
      {
        timeoutMs: 5000,
      }
    ...
    assertEquals(
      messages.length,
      3,
      'there should be 3 archived messages'
    );
    assertEquals(
