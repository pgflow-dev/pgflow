...
import { StepTaskPollerV2, type StepTaskPollerV2Config } from './StepTaskPollerV2.js';
...
/**
 * Configuration for the flow worker V2 with two-phase polling
 */
export type FlowWorkerV2Config = {
  /**
 ...
 */
export function createFlowWorkerV2<TFlow extends AnyFlow>(
  flow: TFlow,
  config: FlowWorkerV2Config,
  createLogger: (module: string) => Logger
): Worker {
  const logger = createLogger('createFlowWorkerV2');
  ...
  if (!config.sql && !config.connectionString) {
    throw new Error(
      "Either 'sql' or 'connectionString' must be provided in FlowWorkerV2Config."
    );
  }
  ...
  // Create StepTaskPollerV2 with two-phase approach
  const pollerConfig: StepTaskPollerV2Config = {
  ...
  const poller = new StepTaskPollerV2<TFlow>(
    pgflowAdapter,
    abortSignal,
    pollerConfig,
    createLogger('StepTaskPollerV2')
  );
